...
/**
 * Trail-only series — distance, elevation gain/loss, pace, average HR —
 * shaped like RunningSeriesService/TrainingVolumeSeriesService so
 * <x-health-trend-chart> works unmodified. Scoped strictly to
 * type='trail_running' so it never mixes in road running, walking, or
 * cycling data.
 */
class TrailSeriesService
{
    ...
    private const METRICS = [
        ...
        'elevation_loss' => ['label' => 'Elevation Loss', 'unit' => 'm', 'color' => '#e34948', 'decimals' => 0],
        'pace' => ['label' => 'Pace', 'unit' => 'min/km', 'color' => '#eb6834', 'decimals' => 2],
        'avg_hr' => ['label' => 'Average HR', 'unit' => 'bpm', 'color' => '#4a3aa7', 'decimals' => 0],
    ];
    ...
    public function seriesFor(User $user, string $metric, int $days): array
    {
        ...
        return match ($metric) {
            ...
            'elevation_loss' => $this->dailySeries($user, $since, 'SUM(elevation_loss_meters)'),
    ...
    public function totalsForRange(User $user, Carbon $from, Carbon $to): array
    {
        ...
        $row = $user->workouts()
            ...
            ->selectRaw(
                'SUM(distance_meters) as distance, COUNT(*) as workout_count, '.
                "SUM(ABS({$durationExpr})) as duration, ".
                'SUM(elevation_gain_meters) as gain, SUM(elevation_loss_meters) as loss, '.
                'AVG(average_pace_seconds_per_km) as pace, AVG(average_heart_rate) as hr'
            )
            ->first();
        ...
        return [
            ...
            'elevation_gain_meters' => (float) ($row->gain ?? 0),
            'elevation_loss_meters' => (float) ($row->loss ?? 0),
     ...
     * (prod) and SQLite (tests). Days where the aggregate is NULL are
     * dropped rather than plotted as 0.
     */
