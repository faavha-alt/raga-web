<x-app-layout>
    <x-slot name="header">
        <h1 class="telemetry-value text-2xl sm:text-3xl lg:text-5xl">{{ __('Suunto') }}</h1>
        <p class="mt-1 text-sm font-medium text-telemetry-slate">Sumber data aktivitas dari Suunto App (read-only).</p>
    </x-slot>
                ...
                <x-card>
                    ...
                    @if ($hasCredentials)
                        ...
                        <a href="{{ route('settings.suunto.connect') }}"
                           class="mt-4 inline-flex items-center justify-center gap-2 min-h-11 sm:min-h-0 rounded border border-telemetry-line-strong bg-telemetry-surface px-5 py-2.5 font-display text-[11px] font-bold uppercase tracking-[0.08em] text-telemetry-ink transition-colors hover:border-telemetry-ink hover:bg-telemetry-well">
