<x-app-layout>
    <x-slot name="header">
        <h1 class="telemetry-value text-2xl sm:text-3xl lg:text-5xl">
            {{ __('Health Trends') }}
        </h1>
        <p class="mt-1 text-sm font-medium text-telemetry-slate">Tren metrik kesehatan kamu dari waktu ke waktu.</p>
    </x-slot>

    <div class="py-4 sm:py-6 pb-16">
        <div class="px-4 sm:px-6 lg:px-8 space-y-3 sm:space-y-6">

            <div class="rounded border border-telemetry-line bg-telemetry-well px-4 py-3 text-xs font-semibold text-telemetry-slate">
                ℹ️ {{ strtolower(\App\Services\Health\PersonalBaselineService::DISCLAIMER) }}
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                @foreach ($series as $key => $data)
                    @php
                        $latest = collect($data['points'])->last();
                        $b = $baselines[$key];
                    @endphp
                    <x-card class="!p-4">
                        <p class="telemetry-label">{{ $data['label'] }}</p>
                        <p class="mt-2 telemetry-value text-xl">
                            {{ $latest ? number_format($latest['value'], $data['decimals']) : '--' }}
                            <span class="text-xs font-semibold text-telemetry-slate">{{ $data['unit'] }}</span>
                        </p>
                        @if ($b)
                            <p class="mt-1 text-[11px] font-semibold text-telemetry-slate">{{ $b['percent_diff'] >= 0 ? '+' : '' }}{{ number_format($b['percent_diff'], 0) }}% vs baseline 30D</p>
                        @endif
                    </x-card>
                @endforeach
            </div>

            <x-card>
                <h3 class="mb-3 telemetry-label-lg text-telemetry-ink">Tren</h3>
                <x-health-trend-chart :series="$series" :ranges="[7, 30, 90, 182, 365]" />
            </x-card>

        </div>
    </div>
</x-app-layout>
